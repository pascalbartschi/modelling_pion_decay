...
def N_Gaussian(t, param, N0=1, mu=0):
    return (N0 / (param[0] - param[1])) * (np.exp(-t / param[0]) - np.exp(-t / param[1])) + \
        norm.pdf(t, loc=mu, scale=param[3])
...
def GaussianSmear_4():
    tVals = randVals(N, 10000, tau)
    for sigma_T in [1 / 100, 1 / 10, tau[1]]:
        # smear with gaussian
        tVals = tVals + np.random.normal(loc=0, scale=sigma_T, size=10000)

        #########################
        # fitting with N(t, param, N0=1)
        ########################

        #########################
        # fitting N_Gaussian(t, sigma, param, N0=1, mu=0)
        #########################