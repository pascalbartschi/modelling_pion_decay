...
def tauEstimates(t_muon, t_muon_uncert, t_pion, t_pion_uncert):
    tVals, tStart, tEnd = randVals(t_muon, t_pion)
    lenData = len(tVals)
    numBins = 30
    counts, edges = np.histogram(tVals, bins=numBins, range=(tStart, tEnd))
    binWidth = (tEnd-tStart)/numBins
    ...
    result = opt.minimize(binnedNNL,x0=tau0, args=(binCentre, tStart, tEnd, numBins, lenData, counts), method="SLSQP",bounds=bounds)
